...
def get_path_to_save_trained_model() -> str:
    current_directory = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(current_directory, "trained_gin_model.pth")
    return model_path
...
def train(train_loader, val_loader, device, model):
    ...
    torch.save(model.state_dict(), get_path_to_save_trained_model())
...
if __name__ == "__main__":
    ...
    with open("Log/GIN/dataset_info.txt", "w") as f:
        for i, l in dataset:
            f.write(str(i) + '\n')
            f.write('label: ' + str(l) + '\n\n\n')
   ...
   # create dataloader
    ...
    # g = dgl.graph(([0, 1, 2, 3, 5], [1, 2, 3, 4, 6]))
    ...
    # # # # Khởi tạo mô hình GIN với 2 lớp ẩn
    # # # model = GIN(input_dim=10, hidden_dim=16, output_dim=16)

    # # # # Chạy qua mô hình để lấy output
    # output = model.forward(g, feat)
    # print(output)